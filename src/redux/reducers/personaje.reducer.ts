...
import { paginarRespuesta } from "../../services/paginarRespuesta";
...
export const personajeReducer: Reducer<PersonajeState, PersonajeAction> = (
  state = initialState,
  action
): PersonajeState => {
  switch (action.type) {
    ...
    case "BUSCAR_PROXIMA_PAGINA_SUCCESS":
      return {
        ...
        personajesPaginas: paginarRespuesta([
          ...state.personajes,
          ...action.personajes,
        ]),
        siguientePagina: action.siguientePagina,
    ...
    case "AGREGAR_FAVORITO":
      return {
        ...
        favoritosPaginas: paginarRespuesta([
          ...state.favoritos.filter(
            (personaje) => personaje.id !== action.personaje.id
          ),
          action.personaje,
        ...
        favoritosId: [
          ...state.favoritosId.filter((id) => id !== action.personaje.id),
          action.personaje.id,
        ],
      };
    case "ELIMINAR_FAVORITO":
      return {
        ...
        favoritosId: state.favoritosId.filter(
          (id) => id !== action.personaje.id
        ),
      };
