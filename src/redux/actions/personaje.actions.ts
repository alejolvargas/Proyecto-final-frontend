...
import {
  buscarPersonajesPorNombreAPI,
  buscarPersonajesPorPaginaAPI,
...
const buscarProximaPaginaSuccess: ActionCreator<
  BuscarPersonajesSuccessAction
> = (personajes: Personaje[], siguientePagina: string) => {
  return {
...
export const buscarProximaPagina: ActionCreator<
  BuscarProximaPaginaAction
> = () => {
  return {
    type: "BUSCAR_PROXIMA_PAGINA",
  ...
  return async (dispatch) => {
    if (name.length > 2 || name.length === 0) {
      ...
      try {
        ...
        dispatch(
          buscarPersonajesSuccess(
            respuesta.personajes,
            respuesta.siguientePagina
          )
        );
      } catch (error) {
